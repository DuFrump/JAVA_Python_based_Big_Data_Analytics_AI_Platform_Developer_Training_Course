# 리스트 컴프리헨션 (List comprehension)
a = [1, 2, 3, 4]
result = []
for num in a:
    result.append(num*3)

print(result) # [3, 6, 9, 12]
print()


print("----- 리스트 컴프리헨션 -----")
a = [1, 2, 3, 4]
result = [num * 3 for num in a if num % 2 == 0]

print(result) # [3, 6, 9, 12]
print()

# [결과 for 항목 in 리스트(튜플) if 문(조건 제약 시)]
# for문을 2개 이상 사용하는 것도 가능함

result = [x * y for x in range(2, 10)
                for y in range(1, 10)]
print(result)
print()


print("----- Break 문 -----")
for i in range(10):
    print(i, end=" ")
    if i == 5:
        break
print()

print("안녕히 주무세요.")  
print()


print("----- for-else 문 -----")
for i in range(5):
    print(i)
else:
    print("for문이 종료되었습니다.")
print()

for i in range(5):
    if i == 3:
        break
    print(i)
else:
    print("for문이 종료되었습니다.")
print()


print("----- enumerate 함수 -----")
fruits = ['apple', 'banana', 'orange']
for fruit in fruits:
    print(fruit)
print()

for i, fruit in enumerate(fruits):
    print(f"{i}번 째 과일: {fruit}")
print()

for i, fruit in enumerate(fruits, 1): # 1부터 시작
    print(f"{i}번 째 과일: {fruit}")
print()


print("----- zip 함수 -----")
names = ['홍길동', '김철수', '이영희']
scores = [85, 93, 56]

# a = zip(names, scores)
# print(a)

# zip은 묶는 함수 ==> [('홍길동', 85), ('김철수', 93) , ('이영희', 56)]
for name, score in zip(names, scores):
    print(f"{name}님의 점수는 {score}점 입니다.")
print()

names = ['홍길동', '김철수', '이영희', '박영수']
scores = [85, 93, 56]
for name, score in zip(names, scores):
    print(f"{name}님의 점수는 {score}점 입니다.")
print()

# zip_longest 임포트를 해야함.
from itertools import zip_longest

for name, score in zip_longest(names, scores, fillvalue = "점수 없음"):
    print(f"{name}: {score}점")
print()

names = ['홍길동', '김철수', '이영희']
korean = [85, 93, 56]
english = [78, 90, 66]
math = [90, 88, 77]

for name, kor, eng, mat in zip(names, korean, english, math):
    total = kor + eng + mat
    avg = total / 3
    print(f"{name}님의 총점은 {total}점, 평균은 {avg:.2f}점입니다. 국어 점수: {kor}, 영어 점수: {eng}, 수학 점수: {mat}")