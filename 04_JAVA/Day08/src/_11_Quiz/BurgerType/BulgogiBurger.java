...
/**
 * 안녕하세요, 두프룸프님! 이번에는 `Hamburger` 클래스를 상속받는 '불고기 버거'를 만들어볼 거예요.
 * `BulgogiBurger` 클래스는 `Hamburger` 클래스의 자식 클래스입니다.
 * 불고기 버거는 기본적인 햄버거의 조리 과정을 따르면서도,
 * 자신만의 특별한 재료(불고기 소스, 치즈)를 추가하여 조리 과정을 '재정의(Overriding)'합니다.
 *
 ...
 *
 * 자, 그럼 맛있는 불고기 버거를 함께 만들어볼까요?
 */
public class BulgogiBurger extends Hamburger {
    // 생성자(Constructor): `BulgogiBurger` 객체를 만들 때 호출됩니다.
    // `super("불고기버거")`는 부모 클래스인 `Hamburger`의 생성자를 호출하는 거예요.
    // 이렇게 하면 `Hamburger` 클래스에서 정의된 `name` 필드를 "불고기버거"로 초기화할 수 있답니다.
    ...
    // 불고기 버거만의 특별한 재료인 치즈와 불고기 소스를 추가로 출력하도록 했습니다.
    @Override
    public void cook() {
