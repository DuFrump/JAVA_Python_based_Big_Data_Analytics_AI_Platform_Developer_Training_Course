/**
 * 안녕하세요, 두프룸프님! 이번에는 'FireDetector' 클래스를 통해
 * 'Detectable' 인터페이스를 어떻게 '구현(implement)'하는지 다시 한번 배워볼 거예요.
 * 'AccidentDetector'나 'AdvancedFireDetector'와 비교하면서 보면
 * 인터페이스 구현의 공통점과 각 감지기의 특화된 기능을 더 명확히 이해할 수 있습니다.
 *
 * <오늘 배울 핵심 개념>
 * 1.  **인터페이스 구현 (`implements`) 복습**: 
 *     - 클래스가 인터페이스의 규약을 따르겠다고 선언할 때 `implements` 키워드를 사용해요.
 *       (예: `public class FireDetector implements Detectable`)
 ...
 *
 * 이 'FireDetector' 클래스는 'Detectable' 인터페이스의 `detect()` 메소드를 구체적으로 구현함으로써,
 * 일반 화재 감지기라는 특정 장치가 '감지' 기능을 어떻게 수행하는지 보여줍니다.
