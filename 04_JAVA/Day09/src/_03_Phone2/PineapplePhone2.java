/**
 * 안녕하세요, 두프룸프님! 이번에는 'PineapplePhone2' 클래스를 통해
 * 'Phone' 인터페이스를 어떻게 '구현(implement)'하는지 자세히 배워볼 거예요.
 * 인터페이스가 제시한 모든 '규약'을 클래스가 어떻게 지켜나가는지 이해하는 데 핵심적인 부분입니다.
 *
 * <오늘 배울 핵심 개념>
 * 1.  **인터페이스 구현 (`implements`)**:
 *     - 클래스가 인터페이스의 규약을 따르겠다고 선언할 때 `implements` 키워드를 사용해요.
 *       (예: `public class PineapplePhone2 implements Phone`)
 ...
 *
 * 이 'PineapplePhone2' 클래스는 'Phone' 인터페이스의 모든 기능을 구체적으로 구현함으로써,
 * 실제 작동하는 휴대폰의 한 종류를 만들어내는 과정을 보여줍니다.
 * 함께 코드를 보면서 인터페이스 구현의 실제 사례를 익혀봅시다!
 */

package _03_Phone2;

// PineapplePhone2 클래스는 Phone 인터페이스를 구현(implements)합니다.
...
public class PineapplePhone2 implements Phone {
    ...
    int batteryCapacity = 40; // 초기 배터리 잔량은 40%로 설정합니다.
    ...
    public void powerOn() {
        if (batteryCapacity > 30) {
            ...
            System.out.println("@@@ Low Battery @@@"); // 배터리가 부족하면 전원을 켤 수 없다는 메시지를 출력합니다.
        }
    }
