...
model.add(keras.layers.Flatten()) # Flatten 층에서 자동으로 펼쳐서 입력
...
train_scaled = train_input / 255.0 # 0 ~ 1 사이의 실수로 스케일링
train_scaled, val_scaled, train_target, val_target = train_test_split(
    train_scaled, train_target, test_size=0.2, random_state=42
)
...
model.compile(optimizer='sgd', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
# 학습률 조정 (학습률이 크면 튀고, 작으면 학습이 느림)
sgd = keras.optimizers.SGD(learning_rate=0.1)
# 모멘텀  (이전 단계의 방향을 일정 비율로 유지하여 진동을 줄이고 빠르게 수렴하도록) 
sgd = keras.optimizers.SGD(momentum=0.9)
# 모멘텀 + 네스테로프 모멘텀
# 더 정확한 방향으로 이동 (한 스탭 미리 보고 이동)
# !!모멘텀 단일 옵션보다 빠르고 안정적인 수렴!!
sgd = keras.optimizers.SGD(momentum=0.9, nesterov=True)

# adagrad - 각 파라미터 별로 학습률을 다르게 조정함.
# 자주 업데이트 되는 파라미터는 학습률을 작게, 드물게 업데이트 되는 파라미터는 학습률을 크게
...
# 빠른 수렴과 자동 학습률 조정을 둘 다 갖고 있는 옵티마이저
