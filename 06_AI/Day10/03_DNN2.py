...
# 강사님 컴퓨터의 학습 결과와 내 컴퓨터 학습 결과가 가능한 똑같이 나오게 해주는 옵션
keras.utils.set_random_seed(42) # 케라스와 관련된 난수 시드 고정
tf.random.set_seed(42) # 텐서 연산 내부에서 사용하는 시드 고정
tf.config.experimental.enable_op_determinism() # 텐서플로우 내부 연산에서 가능한 동일 결과
...
# 모델 생성 함수
def model_fit(a_layer=None): # 매개변수 디폴트값 None
...
# 모델 컴파일 및 훈련
# model.fit을 history에 담기
model.compile(loss='sparse_categorical_crossentropy', metrics=['accuracy'])
print('\n----- 모델 훈련 -----')
history = model.fit(train_scaled, train_target, epochs=5, verbose=0)

# history 객체에 훈련 측정 값이 딕셔너리로 담겨있다.
print('\n----- history key값 확인 -----')
...
# epoch 별 accuracy 값 그래프plt.show()
...
# # 과연 로스 값이 낮으면 무조건 좋을걸까?
# # 검증 세트 돌려서 과대적합 확인해 봐야 한다. 
...
# # 성능 향상이나 과적합 해결의 징후는 크게 없다.

# # ---------- 드롭 아웃 (일반화 성능을 높히는 방법 중 하나) ----------
# # 드롭아웃 층 추가해서 모델 생성
...
# ------------------------------------------------------------------------------------------------------------------
# 20 에포크 동안 할 필요가 없다.
# patience=2 >> 2번 연속 검증 점수가 향상되지 않을 시 훈련 종료
# restore_best_weights=True >> 훈련 중 가장 낮은 검증 손실을 낸 모델 파라미터로 되돌림.
