...
word_vec = TfidfVectorizer(
    analyzer='word', # 디폴트 값, 토큰 단위가 단어이다.
    ngram_range=(1, 2),
    max_df=0.97, # 문서에서 97% 이상 등장하면 제거
    lowercase=True # 소문자 처리
)

print('---------- 단어 백터화 시작 n-gram(1,2) ----------')
print()
...
char_vec = TfidfVectorizer(
    analyzer='char_wb', # 토큰 단위가 문자
    ngram_range=(3, 5),
    min_df=3,
    lowercase=True
)

print('---------- 문자 백터화 시작 n-gram(3,5) ----------')
print()
...
print(f'모델 총 학습 시간: {learning_time:.2f}초')
print()
...
sample_X = hstack([sample_Xw, sample_Xc]).tocsr()
