...
from sklearn.model_selection import train_test_split, RandomizedSearchCV # ✨ RandomizedSearchCV 추가
...
# (이하 키워드 가중치 적용, 라벨 인코딩, SMOTE 오버샘플링 등 기존 코드와 동일)
keyword_weights = {
    '스포츠': ['감독', '경기', '구단', '농구', '득점', '리그', '선수', '승리', '야구', '운동', '월드컵', '올림픽', '응원', '이적', '챔피언', '축구', '훈련'],
    '경제/제테크': ['경제', '금리', '나스닥', '대출', '매도', '매수', '부동산', '세금', '수익', '시장', '예금', '적금', '주가', '주식', '증시', '채권', '투자', '펀드', '환율', 'ETF', '코스피'],
...
param_dist = {
    'n_estimators': [800, 1000, 1200], # learning_rate가 낮아질 수 있으니 트리는 더 많이
    'max_depth': [4, 5, 6],           # 최대 깊이를 낮춰 복잡도 제한
...
# (이하 예측 및 저장 코드는 기존과 동일하며, best_model 변수를 사용하도록 수정)
samples = [
...
predicted_encoded = best_model.predict(samples_vec)
...
new_predicted_encoded = best_model.predict(new_samples_vec)
...
# -------------------------------------------------------------------
print("\n최적화된 모델과 도구들을 저장합니다...")
joblib.dump(best_model, './Project1/models/XGBoost/korean_topic_model.pkl')
